...
#Region "Const of tdbg2 - Total of Columns: 12"
    Private Const COL_DataType As String = "DataType"                 ' DataType
    Private Const COL_Type As String = "Type"                         ' Type
    Private Const COL_Code As String = "Code"                         ' Code
    Private Const COL_FieldName As Integer = 3            ' Diễn giải
    Private Const COL_Description As Integer = 4         ' Tên Tiếng Việt
    Private Const COL_Description01 As Integer = 5       ' Tên Tiếng Anh
    Private Const COL_Short As Integer = 6                      ' Tên tắt
    Private Const COL_NumValue As String = "NumValue"                 ' Số tháng giữ
    Private Const COL_Decimals As String = "Decimals"                 ' Làm tròn
    Private Const COL_Disabled As String = "Disabled"                 ' Sử dụng
    Private Const COL_LastModifyUserID As String = "LastModifyUserID" ' Người cập nhật cuối
    Private Const COL_LastModifyDate As String = "LastModifyDate"     ' Ngày cập nhật cuối
#End Region
    ...
    Private Sub D13F0010_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Loadlanguage()
        UnicodeGridDataField(tdbg1, UnicodeArrayCOL(tdbg1), bUnicode)
        UnicodeGridDataField(tdbg2, UnicodeArrayCOL(tdbg2), bUnicode)
        UnicodeGridDataField(tdbg3, UnicodeArrayCOL(tdbg3), bUnicode)
        UnicodeGridDataField(tdbg4, UnicodeArrayCOL(tdbg4), bUnicode)
        UnicodeGridDataField(tdbg5, UnicodeArrayCOL(tdbg5), bUnicode)
        UnicodeGridDataField(tdbg6, UnicodeArrayCOL(tdbg6), bUnicode)
        UnicodeGridDataField(tdbg7, UnicodeArrayCOL(tdbg7), bUnicode)
        UnicodeGridDataField(tdbg8, UnicodeArrayCOL(tdbg8), bUnicode)
        UnicodeGridDataField(tdbg9, UnicodeArrayCOL(tdbg9), bUnicode)
        UnicodeGridDataField(tdbg10, UnicodeArrayCOL(tdbg10), bUnicode)
        UnicodeGridDataField(tdbg11, UnicodeArrayCOL(tdbg11), bUnicode)
        UnicodeGridDataField(tdbg12, UnicodeArrayCOL(tdbg12), bUnicode)
        UnicodeGridDataField(tdbg13, UnicodeArrayCOL(tdbg13), bUnicode)
        UnicodeGridDataField(tdbg14, UnicodeArrayCOL(tdbg14), bUnicode)
        UnicodeGridDataField(tdbg15, UnicodeArrayCOL(tdbg15), bUnicode)
        tdbg_LockedColumns()

        tdbgX_NumberFormat(tdbg1)
        tdbgX_NumberFormat(tdbg2)
        ...
        InputDateInTrueDBGrid(tdbg14, COL_LastModifyDate)
        InputDateInTrueDBGrid(tdbg15, COL_LastModifyDate)
    ...
    Private Sub LoadGridLanguage(ByVal tdbgX As C1.Win.C1TrueDBGrid.C1TrueDBGrid)
        With tdbgX
            ...
            If tdbgX.Name = "tdbg1" Or tdbgX.Name = "tdbg2" Then .Columns("NumValue").Caption = rL3("So_thang_giu") 'ID 98331 01.08.2017
        End With
    End Sub
    Public Sub New()
        ...
        AnchorForControl(EnumAnchorStyles.TopLeftRightBottom, pnlM, tabMain)
        AnchorResizeColumnsGrid(EnumAnchorStyles.TopLeftRightBottom, tdbg1, tdbg2, tdbg3, tdbg4, tdbg5, tdbg6, tdbg7, tdbg8, tdbg9, tdbg10, tdbg11, tdbg12, tdbg13, tdbg14, tdbg15)
    ...
    Private Sub Loadlanguage()
        ...
        TabPage14.Text = rL3("He_so_don_vi") ' Hệ số đơn vị
        TabPage15.Text = rL3("He_so_to_nhom") ' Hệ số đơn vị
        '================================================================ 
        ...
        LoadGridLanguage(tdbg14)
        LoadGridLanguage(tdbg15)
    End Sub
    ...
    Private Sub tdbgX_NumberFormat(ByVal tdbgX As C1.Win.C1TrueDBGrid.C1TrueDBGrid)
        Dim arr() As FormatColumn = Nothing
        AddDecimalColumns(arr, tdbgX.Columns(COL_NumValue).DataField, DxxFormat.DefaultNumber0, 28, 8)
        InputNumber(tdbgX, arr)
    End Sub
    ...
    Private Sub tdbg_LockedColumns()
        ...
        TdbgX_LockColumn(tdbg14)
        TdbgX_LockColumn(tdbg15)
    End Sub
    ...
    Private Sub LoadTDBGrid()
        If dt.Rows.Count > 0 Then
            ...
            LoadDataSource(tdbg14, ReturnTableFilter(dt, "Type = 'D91T0016'", True), bUnicode)
            LoadDataSource(tdbg15, ReturnTableFilter(dt, "Type = 'D09T0227'", True), bUnicode)
    ...
    Private Function AllowSave() As Boolean
        ...
        If Not CheckColDecimal(tdbg14) Then Return False
        If Not CheckColDecimal(tdbg15) Then Return False
        Return True
    End Function
    ...
    Private Function CheckColDecimal(ByVal tdbg As C1.Win.C1TrueDBGrid.C1TrueDBGrid) As Boolean
        For i As Integer = 0 To tdbg.RowCount - 1
            If tdbg(i, COL_Decimals).ToString = "" And tdbg(i, COL_DataType).ToString = "1" Then
                ...
                tdbg.Col = IndexOfColumn(tdbg, COL_Decimals)
                tdbg.Bookmark = i
                Return False
            End If
        Next
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        sSQL.Append(SQLUpdateD13T9000s(tdbg14).ToString & vbCrLf)

        sSQL.Append(SQLUpdateD13T9000s(tdbg15).ToString & vbCrLf)
...
tdbg7.FetchCellStyle, tdbg8.FetchCellStyle, tdbg9.FetchCellStyle, tdbg10.FetchCellStyle, tdbg11.FetchCellStyle, tdbg12.FetchCellStyle, tdbg13.FetchCellStyle, tdbg14.FetchCellStyle, tdbg15.FetchCellStyle

        Dim tdbgX As C1.Win.C1TrueDBGrid.C1TrueDBGrid = CType(sender, C1.Win.C1TrueDBGrid.C1TrueDBGrid)
        With tdbgX
            Select Case .Col
                Case IndexOfColumn(tdbgX, COL_Decimals)
...
tdbg2.HeadClick, tdbg3.HeadClick, tdbg4.HeadClick, tdbg5.HeadClick, tdbg6.HeadClick, _
        tdbg7.HeadClick, tdbg8.HeadClick, tdbg9.HeadClick, tdbg10.HeadClick, tdbg11.HeadClick, tdbg12.HeadClick, tdbg13.HeadClick, tdbg14.HeadClick, tdbg15.HeadClick
        ...
        With tdbgX
            ...
            Select Case e.ColIndex
                Case IndexOfColumn(tdbgX, COL_Disabled)
...
tdbg7.KeyDown, tdbg8.KeyDown, tdbg9.KeyDown, tdbg10.KeyDown, tdbg11.KeyDown, tdbg12.KeyDown, tdbg13.KeyDown, tdbg14.KeyDown, tdbg15.KeyDown
        Dim tdbgX As C1.Win.C1TrueDBGrid.C1TrueDBGrid = CType(sender, C1.Win.C1TrueDBGrid.C1TrueDBGrid)
        With tdbgX
            If e.KeyCode = Keys.Enter Then
                If .Col = IndexOfColumn(tdbgX, COL_Disabled) Then
                    HotKeyEnterGrid(tdbgX, IndexOfColumn(tdbgX, COL_Description), e)
                End If
            End If
        End With
    End Sub
    ...
    Private Function SQLUpdateD13T9000s(ByVal c1Grid As C1.Win.C1TrueDBGrid.C1TrueDBGrid) As StringBuilder
        ...
        For i As Integer = 0 To c1Grid.RowCount - 1
            ...
            If c1Grid.Name = "tdbg1" Or c1Grid.Name = "tdbg2" Then sSQL.Append(COMMA & "NumValue = " & SQLNumber(c1Grid(i, COL_NumValue))) 'datetime, NULL
            sSQL.Append(" Where ")
    ...
    Private Function UnicodeArrayCOL(c1Grid As C1.Win.C1TrueDBGrid.C1TrueDBGrid) As Integer()
