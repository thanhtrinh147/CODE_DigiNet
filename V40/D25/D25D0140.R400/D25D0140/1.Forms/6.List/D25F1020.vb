Public Class D25F1020

#Region "Const of tdbg"
    Private Const COL_RecPositionID As Integer = 0    ' Mã
    Private Const COL_RecPositionName As Integer = 1  ' Diễn giải
...
#End Region
    ...
    Private Sub D25F1020_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
        If e.KeyCode = Keys.Enter Then
            UseEnterAsTab(Me)
        End If
    End Sub

    Private Sub D25F1020_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
	LoadInfoGeneral()
...
SetResolutionForm(Me, ContextMenuStrip1)
    ...
    Private Sub Loadlanguage()
        '================================================================ 
        Me.Text = rl3("Danh_muc_vi_tri_ung_tuyen_-_D25F1020") & UnicodeCaption(gbUnicode) 'Danh móc vÜ trÛ ÷ng tuyÓn - D25F1020
        '================================================================ 
        tdbg.Columns("RecPositionID").Caption = rl3("Ma") 'Mã
        tdbg.Columns("RecPositionName").Caption = rl3("Dien_giai") 'Diễn giải
    ...
    Private Sub LoadTDBGrid(Optional ByVal bFlagAdd As Boolean = False, Optional ByVal sKey As String = "")
        Dim sSQL As String = ""
        sSQL &= " Select RecPositionID, RecPositionName" & UnicodeJoin(gbUnicode) & " as RecPositionName, Disabled," & vbCrLf
        sSQL &= " CreateUserID,CreateDate,LastModifyUserID,LastModifyDate " & vbCrLf
        sSQL &= " From 	D25T1020  WITH(NOLOCK)  Order by 	RecPositionID" & vbCrLf
        ...
        If sKey <> "" Then 'Khi Thêm mới hoặc Sửa đều thực thi
            ...
            Dim dr() As DataRow = dt.Select(tdbg.Columns(COL_RecPositionID).DataField & "=" & SQLString(sKey), dt.DefaultView.Sort)
...
#Region "Active FilterChange "
    ...
    Private Sub ResetGrid()
        FooterTotalGrid(tdbg, COL_RecPositionID)
...
#End Region
    ...
    Private Sub tsbAdd_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles tsbAdd.Click, tsmAdd.Click, mnsAdd.Click
        Try
            Dim f1 As New D25F1021
            f1.RecPositionID = ""
            f1.FormState = EnumFormState.FormAdd
            f1.ShowDialog()
            skey = f1.RecPositionID
            f1.Dispose()
            If f1.bSaved Then
    ...
    Private Sub tsbView_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles tsbView.Click, tsmView.Click, mnsView.Click
        Dim f1 As New D25F1021
        f1.RecPositionID = tdbg.Columns(COL_RecPositionID).Text
    ...
    Private Sub tsbEdit_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles tsbEdit.Click, tsmEdit.Click, mnsEdit.Click
        ...
        f1.ShowDialog()
        If f1.bSaved Then
            LoadTDBGrid(False, tdbg.Columns(COL_RecPositionID).Text)
        End If
        f1.Dispose()
    End Sub
    ...
    Private Sub tsbDelete_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles tsbDelete.Click, tsmDelete.Click, mnsDelete.Click
        ...
        Dim sSQL As String = ""
        sSQL = "Delete D25T1020 Where RecPositionID = " & SQLString(tdbg.Columns("RecPositionID").Text)
    ...
    Private Sub tsbInherit_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles tsbInherit.Click, tsmInherit.Click, mnsInherit.Click
        Dim f1 As New D25F1021
        f1.RecPositionID = tdbg.Columns(COL_RecPositionID).Text
        f1.FormState = EnumFormState.FormCopy
        f1.ShowDialog()
        skey = f1.RecPositionID
        f1.Dispose()
        If f1.bSaved Then
    ...
    Private Function AllowDelete() As Boolean
        ...
        If IsExistKey("D25T1041", "RecPositionID", tdbg.Columns(COL_RecPositionID).Text) Then
            D99C0008.MsgL3(rl3("Vi_tri_tuyen_dung_nay_da_duoc_su_dung_roi"))
            Return False
        ElseIf IsExistKey("D25T1042", "RecPositionID", tdbg.Columns(COL_RecPositionID).Text) Then
            D99C0008.MsgL3(rl3("Vi_tri_tuyen_dung_nay_da_duoc_su_dung_roi"))
            Return False
        ElseIf IsExistKey("D25T2001", "RecPositionID", tdbg.Columns(COL_RecPositionID).Text) Then
            D99C0008.MsgL3(rl3("Vi_tri_tuyen_dung_nay_da_duoc_su_dung_roi"))
            Return False
        ElseIf IsExistKey("D25T2031", "RecPositionID", tdbg.Columns(COL_RecPositionID).Text) Then
            D99C0008.MsgL3(rl3("Vi_tri_tuyen_dung_nay_da_duoc_su_dung_roi"))
            Return False
        End If
        Return True
