...
'# Ngày tạo: 29/11/2010 
'# Ngày cập nhật cuối cùng:  21/2/2014
...
#Region "Khai báo biến toàn cục"
    ...
    Public gbyD91InvoicesDateInPeriod As Integer = -1
...
#Region "Kiểm tra ngày hóa đơn lớn hơn ngày phiếu"
    ''' <summary>
    ''' Kiểm tra ngày hóa đơn có phù hợp với kỳ hiện tại , Kiểm tra ngày chứng từ và Ngày hóa đơn
    ''' </summary>
            ...
            If CDate(SQLDateShow(c1RefDate.Value.ToString)) < CDate(SQLDateShow(gdDatePeriodFrom.Date)) Or CDate(SQLDateShow(c1RefDate.Value.ToString)) > CDate(SQLDateShow(gdDatePeriodTo.Date)) Then 'Vi phạm
                Select Case gbyD91InvoicesDateInPeriod
                    Case 1 ' Kiểm tra thông báo
                        ...
                        If D99C0008.MsgAsk(r("MSG000054") & vbCrLf & r("MSG000021")) = Windows.Forms.DialogResult.No Then
                            If tabSelection IsNot Nothing AndAlso Index <> -1 Then tabSelection.SelectedIndex = Index
                            c1RefDate.Focus()
                            Return False
                        End If
                    Case 2 ' Kiểm tra không cho lưu
                        D99C0008.MsgL3(r("MSG000054") & vbCrLf & r("MSG000053"), L3MessageBoxIcon.Exclamation)
                        If tabSelection IsNot Nothing AndAlso Index <> -1 Then tabSelection.SelectedIndex = Index
                        c1RefDate.Focus()
                        Return False
                End Select
    ...
    Public Sub GetD91RefDateCheck(ByVal sModule As String)
        ...
        Dim dtCheck As DataTable = ReturnDataTable("Exec D91P9301 " & SQLString(sModule))
        gbyD91RefDate = CByte(dtCheck.Rows(0).Item("RefDateCheck"))
        gbyD91InvoicesDateInPeriod = CByte(dtCheck.Rows(0).Item("InvoicesDateInPeriod"))
