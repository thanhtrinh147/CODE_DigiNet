    ...
    Private Sub btnAttendance_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnAttendance.Click
        Dim iPieceWorkMethod As Integer
            ...
            If iType = 1 Then
                'id -  126396
                Dim sSQL1 As String
                sSQL1 = "SELECT TOP 1 1 FROM 	D09T0009" & vbCrLf
                sSQL1 &= "WHERE 	TransTypeID = 'NewForm' AND NumValue = 1 AND " & SQLString(gsDivisionID) & " NOT IN ( 'NHI', 'NDV', 'NHP', 'NDP', 'PCF', 'DQP' )"
                Dim dt1 As DataTable = ReturnDataTable(sSQL1)
                If dt1.Rows.Count > 0 Then
                    If dt1.Rows(0)(0).ToString = "1" Then
                        Dim f As New D45F2016
                        f.ProductVoucherID = _productVoucherID
                        f.ProductVoucherNo = txtProductVoucherNo.Text
                        f.VoucherDate = c1dateVoucherDate.Value.ToString 'Format(c1dateVoucherDate.Value, "dd/MM/yyyy")
                        f.Note = txtNote.Text
                        f.PayrollVoucherID = gsPayrollVoucherID
                        f.BlockID = ReturnValueC1Combo(tdbcBlockID)
                        f.DepartmentID = ReturnValueC1Combo(tdbcDepartmentID)
                        f.TeamID = ReturnValueC1Combo(tdbcTeamID)
                        f.TransTypeID = ReturnValueC1Combo(tdbcTransTypeID)
                        f.ShowDialog()
                        f.Dispose()
                    End If                  
                Else
            ...
            ElseIf iPieceWorkMethod = 1 OrElse iPieceWorkMethod = 3 Then
                ...
                With f
                    ...
                    .ProductVoucherNo = txtProductVoucherNo.Text
                    .VoucherDate = Format(c1dateVoucherDate.Value, "dd/MM/yyyy")
                    .sMethod = iPieceWorkMethod.ToString
