...
Public Class D45F2007
    ...
    Dim bKeyPress As Boolean = False, bNotInList As Boolean = False
...
#Region "Const of tdbg - Total of Columns: 56"
    ...
    Private Const COL_Notes As Integer = 55           ' Ghi chú
#End Region
...
#End Region
    ...
    Public WriteOnly Property bSaved() As Boolean 
        Set(ByVal Value As Boolean )
            _bSaved = Value
        End Set
    End Property
    ...
    Private Sub D45F2007_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles Me.KeyPress
        bKeyPress = True
    End Sub
...
#Region "Events tdbcDepartmentID"
    ...
    Private Sub tdbc_GotFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles tdbcDepartmentID.GotFocus, tdbcTeamID.GotFocus
    ...
    Private Sub tdbcDepartmentID_SelectedValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles tdbcDepartmentID.SelectedValueChanged
        'LoadtdbcTeamID(CbVal(tdbcDepartmentID))
        LoadtdbcTeamID(tdbcTeamID, dtTeamID, ReturnValueC1Combo(tdbcBlockID).ToString, ReturnValueC1Combo(tdbcDepartmentID).ToString, gbUnicode)
...
#Region "53.Sửa lỗi gõ tên trên combo hay dropdown"
    Private Sub tdbcName_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles tdbcDepartmentID.LostFocus, tdbcTeamID.LostFocus
        ...
        If CbVal(tdbcName) <> tdbcName.Tag.ToString Then LoadTDBGrid()
    End Sub

    Private Sub tdbcName_Close(ByVal sender As Object, ByVal e As System.EventArgs) Handles tdbcDepartmentID.Close, tdbcTeamID.Close, tdbcDepartmentID.Validated
        tdbcName_Validated(sender, Nothing)
    End Sub

    Private Sub tdbcName_Validated(ByVal sender As Object, ByVal e As System.EventArgs) Handles tdbcTeamID.Validated
...
#End Region
    ...
    Private Sub LoadTDBGrid()
        ...
        If CbVal(tdbcDepartmentID) <> "%" Then
            ...
            'tdbg.Columns(COL_DepartmentName).DefaultValue = tdbcDepartmentID.Text
        Else
            tdbg.Splits(0).DisplayColumns(COL_DepartmentID).Visible = True
            tdbg.Columns(COL_DepartmentID).DefaultValue = ""
            'tdbg.Columns(COL_DepartmentName).DefaultValue = ""
        End If

        If CbVal(tdbcTeamID) <> "%" Then
            tdbg.Columns(COL_TeamID).DefaultValue = CbVal(tdbcTeamID)
            tdbg.Splits(0).DisplayColumns(COL_TeamID).Visible = False
            'tdbg.Splits(0).DisplayColumns(COL_TeamID).AllowFocus = False
            'tdbg.Columns(COL_TeamName).DefaultValue = tdbcTeamID.Text
        Else
            tdbg.Columns(COL_TeamID).DefaultValue = ""
            tdbg.Splits(0).DisplayColumns(COL_TeamID).Visible = True
            'tdbg.Splits(0).DisplayColumns(COL_TeamID).AllowFocus = True
            'tdbg.Columns(COL_TeamName).DefaultValue = ""
        End If
...
#Region "Luoi"
    ...
    Private Sub tdbg_BeforeColUpdate(ByVal sender As System.Object, ByVal e As C1.Win.C1TrueDBGrid.BeforeColUpdateEventArgs) Handles tdbg.BeforeColUpdate
        ...
        Select Case e.ColIndex
            Case COL_DepartmentID
                If tdbg.Columns(e.ColIndex).Text <> tdbdDepartmentID.Columns(tdbdDepartmentID.DisplayMember).Text Then
                    'tdbg.Columns(COL_DepartmentID).Text = ""
...
#End Region
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        sSQL.Append(SQLInsertD45T2001s.ToString)
    ...
    Private Function SQLInsertD45T2001s() As StringBuilder
        ...
        For i As Integer = 0 To tdbg.RowCount - 1
            ...
            sSQL.Append("RefSalProduct01, RefSalProduct02, RefSalProduct03, RefSalProduct04, RefSalProduct05,MachineID,NotesU")
            sSQL.Append(") Values(")
            ...
            'ID 94408 20.01.2017
            sSQL.Append(SQLStringUnicode(tdbg(i, COL_Notes), True, True)) 'NoteU, varchar[20], NOT NULL
            sSQL.Append(")")
