...
#Region "Const of tdbg2 - Total of Columns: 15"
    ...
    Private Const COL2_DepartmentName As Integer = 9       ' Tên phòng ban
    Private Const COL2_TeamID As Integer = 10              ' Tổ nhóm
    Private Const COL2_TeamName As Integer = 11            ' Tên tổ nhóm
    Private Const COL2_WorkingHours As Integer = 12        ' Số giờ làm việc
    Private Const COL2_MasterApportionCoef As Integer = 13 ' Tổng số giờ làm việc

    Private Const COL2_Total As Integer = 14
#End Region
...
#End Region
    ...
    Private Sub Loadlanguage()
        ...
        tdbg2.Columns("DepartmentName").Caption = rl3("Ten_phong_ban")'Tên phòng ban
        tdbg2.Columns("TeamID").Caption = rL3("To_nhom") 'Tổ nhóm
        tdbg2.Columns("TeamName").Caption = rl3("Ten_to_nhom")'Tên tổ nhóm
    ...
    Private Sub LoadTDBDropDown()
        ...
        sSQL = "SELECT T1.EmployeeID AS EmployeeID, T2.RefEmployeeID AS RefEmployeeID, " & vbCrLf
         If gbUnicode = False Then
            sSQL &= "Isnull(T2.LastName,'') + ' ' + Isnull(T2.MiddleName,'') + ' ' + Isnull(T2.FirstName,'') as EmployeeName, "
        Else
            sSQL &= "Isnull(T2.LastNameU,'') + ' ' + Isnull(T2.MiddleNameU,'') + ' ' + Isnull(T2.FirstNameU,'') as EmployeeName, "
        End If
        sSQL &= "T2.FirstName" & UnicodeJoin(gbUnicode) & " As FirstName, " & vbCrLf 
        sSQL &= "T1.DepartmentID, " & IIF(gbUnicode ,"T3.DepartmentNameU","T3.DepartmentName").ToString () & " AS DepartmentName," & vbCrLf
        sSQL &= "T1.TeamID, " & IIF(gbUnicode,"T4.TeamNameU","T4.TeamName").ToString() & " AS TeamName, T2.BlockID" & vbCrLf 
        sSQL &= "FROM  D13T0101 T1 WITH (NOLOCK) " & vbCrLf
        sSQL &= "INNER JOIN	D09T0201 T2  WITH(NOLOCK) ON T1.EmployeeID = T2.EmployeeID " & vbCrLf
        sSQL &= "INNER JOIN	D91T0012 T3 WITH (NOLOCK) ON T1.DepartmentID = T3.DepartmentID " & vbCrLf
        sSQL &= "LEFT JOIN	D09T0227 T4 WITH (NOLOCK) ON T1.TeamID = T4.TeamID " & vbCrLf
        sSQL &= "WHERE		T1.DivisionID = " & SQLString(gsDivisionID) & " And PayrollVoucherID = "  & SQLString(_payrollVoucherID)
...
#End Region
    ...
    Private Sub LoadTDBGrid2()
        ...
        If dtGrid2.Columns.Contains("DepartmentName") = False Then '118512 - KhaVi - 10 March 2019
            dtGrid2.Columns.Add("DepartmentName", Type.GetType("System.String"))
        End If
        If dtGrid2.Columns.Contains("TeamName") = False Then
            dtGrid2.Columns.Add("TeamName", Type.GetType("System.String"))
        End If
        ...
        Select Case e.ColIndex
            Case COL2_Choose
                ...
                Else
                    ...
                    tdbg2.Columns(COL2_Choose).Tag = tdbg2.Row 
                End If
    ...
    Private Sub tdbg2_ComboSelect(ByVal sender As Object, ByVal e As C1.Win.C1TrueDBGrid.ColEventArgs) Handles tdbg2.ComboSelect
        Select Case e.ColIndex
            Case COL2_EmployeeID
                ...
                tdbg2.Columns(COL2_DepartmentName).Text = tdbdEmployeeID.Columns("DepartmentName").Text
                tdbg2.Columns(COL2_TeamID).Text = tdbdEmployeeID.Columns("TeamID").Text
                tdbg2.Columns(COL2_TeamName).Text = tdbdEmployeeID.Columns("TeamName").Text
                FooterTotalGrid(tdbg2, COL2_EmployeeID)
            ...
            Case COL2_RefEmployeeID
                ...
                tdbg2.Columns(COL2_DepartmentName).Text = tdbdRefEmployeeID.Columns("DepartmentName").Text
                tdbg2.Columns(COL2_TeamID).Text = tdbdRefEmployeeID.Columns("TeamID").Text
                tdbg2.Columns(COL2_TeamName).Text = tdbdRefEmployeeID.Columns("TeamName").Text
                FooterTotalGrid(tdbg2, COL2_EmployeeID)
            ...
            Case COL2_FirstName
                ...
                tdbg2.Columns(COL2_DepartmentName).Text = tdbdFirstName.Columns("DepartmentName").Text
                tdbg2.Columns(COL2_TeamID).Text = tdbdFirstName.Columns("TeamID").Text
                tdbg2.Columns(COL2_TeamName).Text = tdbdFirstName.Columns("TeamName").Text
                FooterTotalGrid(tdbg2, COL2_EmployeeID)
        End Select
    End Sub
    ...
    Private Function SaveData_Detail() As StringBuilder
        ...
        If chkIsPieceworkGroup.Checked = False Then
            ...
            sRet.Append(SQLStoreD45P2047(0) & vbCrLf)
            For i As Integer = 0 To tdbg2.RowCount - 1
                k = 0
                While k < nTotalProduct1
                    ...
                    If L3Bool (tdbg2(i,COL2_Choose).ToString ) AndAlso (Number(tdbg2(i, tdbg2.Columns(iCol).DataField).ToString) <> 0 OrElse Number(tdbg2(i, tdbg2.Columns(iCol + 1).DataField).ToString) <> 0 OrElse Number(tdbg2(i, tdbg2.Columns(iCol + 2).DataField).ToString) <> 0 _
        ...
        Else
            ...
            'sRet.Append(SQLDeleteD45T2001.ToString & vbCrLf)
            sRet.Append(SQLStoreD45P2047(1) & vbCrLf)
            'Luu D45T2002
    ...
    Private Function SaveData_Master() As StringBuilder
        ...
        Else
            ...
            'sRet.Append(SQLDeleteD45T2002.ToString & vbCrLf)
            'sRet.Append(SQLDeleteD45T2001.ToString & vbCrLf)
            sRet.Append(SQLStoreD45P2047(1).ToString & vbCrLf)
    ...
    '#---------------------------------------------------------------------------------------------------
'# Title: SQLStoreD45P2047
'# Created User: 
'# Created Date: 13/05/2019 04:06:35
'#---------------------------------------------------------------------------------------------------
    Private Function SQLStoreD45P2047(_mode As Integer) As String
        Dim sSQL As String = ""
        sSQL &= ("-- DELETE bang D45T2001, D45T2002" & vbCrLf)
        sSQL &= "Exec D45P2047 "
        sSQL &= SQLString(_productVoucherID) & COMMA 'ProductVoucherID, varchar[20], NOT NULL
        sSQL &= SQLString(gsDivisionID) & COMMA 'DivisionID, varchar[20], NOT NULL
        ...
        Dim k As Integer = 0
        Try
            If _mode = 0 Then 'mode không check thống kê theo nhóm CCSP
        
                For i As Integer = 0 To tdbg2.RowCount - 1
                    k = 0
                    While k < nTotalProduct1
                        iCol = COL2_Total + (k * 6) '6 cột cho 1 công đoạn
                        If L3Bool(tdbg2(i, COL2_Choose)) AndAlso (Number(tdbg2(i, tdbg2.Columns(iCol).DataField).ToString) <> 0 OrElse Number(tdbg2(i, tdbg2.Columns(iCol + 1).DataField).ToString) <> 0 OrElse Number(tdbg2(i, tdbg2.Columns(iCol + 2).DataField).ToString) <> 0 _
                            OrElse Number(tdbg2(i, tdbg2.Columns(iCol + 3).DataField).ToString) <> 0 OrElse Number(tdbg2(i, tdbg2.Columns(iCol + 4).DataField).ToString) <> 0 OrElse Number(tdbg2(i, tdbg2.Columns(iCol + 5).DataField).ToString) <> 0) Then
                            sEmployeeID &= "<Data><EmployeeID>" & tdbg2(i, COL2_EmployeeID).ToString & "</EmployeeID></Data>"
                        End If
                        k += 1
                    End While
                Next
            Else
                If optMasterApportionCoef.Checked = True Then
                    For i As Integer = 0 To tdbg2.RowCount - 1
                        If Number(tdbg2(i, COL2_MasterApportionCoef)) <> 0 Then
                            'Sinh IGE cho TransID
                            sEmployeeID &= "<Data><EmployeeID>" & tdbg2(i, COL2_EmployeeID).ToString & "</EmployeeID></Data>"
        ...
        Catch ex As Exception
            MessageBox.Show(ex.Message)
        End Try
        sSQL &= SQLString(sEmployeeID) 'XML , varchar[20], NOT NULL
        Return sSQL
    End Function
    ...
    Private Function ExecuteSQL_MoreCommand(ByVal strSQL As String) As Boolean
        ...
        'If gbUnicode Then strSQL = "SET ANSI_WARNINGS OFF " & vbCrLf & strSQL '118512 - KieuVan - 17 May 2019
    ...
    Private Sub tdbg2_RowColChange(ByVal sender As Object, ByVal e As C1.Win.C1TrueDBGrid.RowColChangeEventArgs) Handles tdbg2.RowColChange
        ...
        Else
            ...
            tdbg2.Columns(COL2_FirstName).DropDown = Nothing
        End If
        '118512 - Khả Vi  - 13 May 2019
        'If L3Bool(tdbg2(e.LastRow, COL2_Choose).ToString) = True And e.LastRow <> tdbg2.Row  Then
        '    Try
        '        Dim k As Integer = 0
        '        Dim iCol As Integer = COL2_Total + (k * 6) '6 cột cho 1 công đoạn
        '        If Not (Number(tdbg2(e.LastRow ,tdbg2.Columns(iCol).DataField).ToString) <> 0 OrElse Number(tdbg2(e.LastRow ,tdbg2.Columns(iCol + 1).DataField).ToString) <> 0 OrElse Number(tdbg2(e.LastRow ,tdbg2.Columns(iCol + 2).DataField).ToString) <> 0 _
        '                  OrElse Number(tdbg2(e.LastRow ,tdbg2.Columns(iCol + 3).DataField).ToString) <> 0 OrElse Number(tdbg2(e.LastRow ,tdbg2.Columns(iCol + 4).DataField).ToString) <> 0 OrElse Number(tdbg2(e.LastRow ,tdbg2.Columns(iCol + 5).DataField).ToString) <> 0) Then
...
#Region "UserControl D09U1111 và Xuất Excel (gồm 7 bước)"
    ...
    Private Sub CallD09U1111_Button(ByVal bLoadFirst As Boolean)
        ...
        If bLoadFirst = True Then
            ...
            Dim arrColObligatory() As Integer = {COL2_Choose , COL2_EmployeeID}
            '-----------------------------------
            'Các cột ở SPLIT0
            AddColVisible(tdbg2, SPLIT0, arrMaster, arrColObligatory, , , gbUnicode)
            'AddColVisible(tdbg2, SPLIT1, arrMaster, arrColObligatory, , , gbUnicode)
        End If
        dtCaptionCols = CreateTableForExcel(tdbg2, arrMaster)
        If usrOption IsNot Nothing Then usrOption.Dispose()
        usrOption = New D09U1111(tdbg2, dtCaptionCols, Me.Name.Substring(1, 2), Me.Name, "0", , bLoadFirst, , gbUnicode)
    End Sub
    ...
    Private Sub btnF12_Click(sender As Object, e As EventArgs) Handles btnF12.Click
        ...
        usrOption.Location = New Point(tdbg2.Left, tdbg2.Top + tdbg2.Height - (usrOption.Height))
        Me.Controls.Add(usrOption)
        usrOption.Parent = pnl1
