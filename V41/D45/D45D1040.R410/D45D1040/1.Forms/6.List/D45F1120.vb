...
#Region "Const of tdbg - Total of Columns: 16"
    ...
    Private Const COL_SubTaskID As String = "SubTaskID"               ' Tiểu tác
    Private Const COL_SubTaskName As String = "SubTaskName"           ' Tên tiểu tác
    ...
    Private Const COL_WorkerLevelID As String = "WorkerLevelID"       ' WorkerLevelID
    Private Const COL_PPNorm As String = "PPNorm"                     ' PPNorm
    Private Const COL_PPUnitPrice As String = "PPUnitPrice"           ' PPUnitPrice
...
#End Region
    ...
    Private _formIDPermission As String = "D45F1120"
    ...
    Public WriteOnly Property TaskID As String
        Set(ByVal Value As String)
            _taskID = Value
        End Set
    End Property

    Private _SubTaskID As String = ""
    Public Property SubTaskID As String
        Get
            Return _SubTaskID
        End Get
        Set(ByVal Value As String)
    ...
    Private Sub D57F1000_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        CheckIdTextBox(txtSubTaskID)
        CheckIdTextBox(txtWorkerLevelID)
        Dim arrC1Number() As C1.Win.C1Input.C1NumericEdit = {cnePPNorm, cnePPUnitPrice}
        InputNumber(arrC1Number, SqlDbType.Decimal, DxxFormat.DefaultNumber2, False, 19, 4) 'Nhập số âm = True. Default = False
    ...
    Private Sub SetBackColorObligatory()
        ...
        tdbcTaskIDM.EditorBackColor = COLOR_BACKCOLOROBLIGATORY
        ...
        txtSubTaskID.BackColor = COLOR_BACKCOLOROBLIGATORY
        txtSubTaskName.BackColor = COLOR_BACKCOLOROBLIGATORY
    End Sub
    Private Sub LoadLanguage()
        '================================================================ 
        Me.Text = rl3("Danh_muc_tieu_tac") & " - " & Me.Name & UnicodeCaption(gbUnicode) 'Danh móc tiÓu tÀc
        '================================================================ 
        lblTaskID.Text = rl3("Cum_tieu_tac") 'Cụm tiểu tác
        lblSubTaskID.Text = rl3("Ma") 'Mã
        lblNotes.Text = rl3("Ghi_chu") 'Ghi chú
        lblSubTaskName.Text = rl3("Ten") 'Tên
        ...
        lblWorkerLevelID.Text = rl3("Bac_tho") 'Bậc thợ
        lblPPNorm.Text = rl3("Dinh_muc") 'Định mức
        lblPPUnitPrice.Text = rl3("Don_gia") 'Đơn giá
        lblGroupProductIDM.Text = rl3("Nhom_san_pham") 'Nhóm sản phẩm
        lblComponentIDM.Text = rl3("Nhom_tieu_tac") 'Nhóm tiểu tác
        lblTaskIDM.Text = rl3("Cum_tieu_tac") 'Cụm tiểu tác
        '================================================================ 
        ...
        tdbcTaskIDM.Columns("TaskID").Caption = rl3("Ma") 'Mã
        tdbcTaskIDM.Columns("TaskName").Caption = rl3("Ten") 'Tên
        ...
        tdbg.Columns(COL_SubTaskID).Caption = rl3("Tieu_tac") 'Tiểu tác
        tdbg.Columns(COL_SubTaskName).Caption = rl3("Ten_tieu_tac") 'Tên tiểu tác
    ...
    Private Sub LoadTDBCombo()
        ...
        sSQL &= "	WHERE		Disabled=0 And TypeCode = '48'" & vbCrLf
        ...
        ''Load tdbcTaskIDM
        'LoadTDBCTaskID(tdbcTaskID)

        'Load tdbcTaskID
        LoadTDBCTaskID(tdbcTaskID, ReturnValueC1Combo(tdbcComponentID))
    End Sub
    ...
    Private Sub LoadTDBCTaskID(tdbc As C1.Win.C1List.C1Combo, sComponentID As String, Optional sTaskID As String = "%")
        If dtTaskID Is Nothing Then
            Dim sSQL As String = ""
            'Load tdbcTaskID
            sSQL = "-- Combo cum tieu tac" & vbCrLf
            sSQL &= "	SELECT  		'+' AS TaskID, " & NewName & "  AS  TaskName, '' As ComponentID, 0 AS DisplayOrder" & vbCrLf
            sSQL &= "	UNION " & vbCrLf
        ...
        End If
        Dim sWhere As String = ""
        If tdbc.Name = tdbcTaskID.Name Then
            sWhere = "TaskID <> '%'"
            If sComponentID <> "" Then
                LoadDataSource(tdbc, ReturnTableFilter(dtTaskID, sWhere & " AND (TaskID ='+' OR  ComponentID =" & SQLString(sComponentID) & ")", True), gbUnicode)
            Else
                LoadDataSource(tdbc, ReturnTableFilter(dtTaskID, sWhere & " AND TaskID =" & SQLString(sComponentID), True), gbUnicode)
            End If
        Else
            sWhere = "TaskID <> '+'"
            LoadDataSource(tdbc, ReturnTableFilter(dtTaskID, sWhere & " AND (TaskID ='%' OR ComponentID =" & SQLString(sComponentID) & ")", True), gbUnicode)
            tdbcTaskIDM.SelectedValue = sTaskID
        End If
    End Sub
...
#Region "Events tdbcComponentIDM"
    ...
    Private Sub tdbcComponentIDM_SelectedValueChanged(sender As Object, e As EventArgs) Handles tdbcComponentIDM.SelectedValueChanged
        LoadTDBCTaskID(tdbcTaskIDM, ReturnValueC1Combo(tdbcComponentIDM), "%")
    End Sub
#End Region
...
#Region "Events tdbcTaskIDM"
    Private Sub tdbcTaskIDM_LostFocus(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles tdbcTaskIDM.LostFocus
        If tdbcTaskIDM.FindStringExact(tdbcTaskIDM.Text) = -1 Then tdbcTaskIDM.Text = ""
    End Sub
#End Region
...
#Region "Events tdbcTaskID"
    ...
    End Sub
#End Region
    Private Sub tdbcName_Close(ByVal sender As Object, ByVal e As System.EventArgs) Handles tdbcGroupProductID.Close, tdbcGroupProductIDM.Close, tdbcMethodID.Close, tdbcComponentIDM.Close, tdbcComponentID.Close, tdbcTaskIDM.Close, tdbcTaskID.Close
        tdbcName_Validated(sender, Nothing)
    End Sub

    Private Sub tdbcName_Validated(ByVal sender As Object, ByVal e As System.EventArgs) Handles tdbcGroupProductID.Validated, tdbcGroupProductIDM.Validated, tdbcMethodID.Validated, tdbcComponentIDM.Validated, tdbcComponentID.Validated, tdbcTaskIDM.Validated, tdbcTaskID.Validated
        ...
        Select Case tdbc.Name
            Case tdbcMethodID.Name
                If tdbcMethodID.Text <> "" Then
                    ReadOnlyControl(txtSubTaskID)
                Else
                    UnReadOnlyControl(True, txtSubTaskID)
                End If
            ...
            Case tdbcComponentID.Name
                ...
                End If
            Case tdbcTaskID.Name
                Dim sTaskID As String = ReturnValueC1Combo(tdbcTaskID)
                If tdbcTaskID.Tag Is Nothing OrElse tdbcTaskID.Tag.ToString <> sTaskID Then
                    If ReturnValueC1Combo(tdbcTaskID) = "+" Then
                        tdbcTaskID.SelectedValue = ""
                        If ReturnPermission("D45F1100") < EnumPermission.Add Then
                            D99C0008.MsgL3(rL3("Ban_khong_co_quyen_them_moi"))
                        Else
                            Dim f As New D45F1100
                            f.GroupProductID = ReturnValueC1Combo(tdbcGroupProductID)
                            f.ComponentID = ReturnValueC1Combo(tdbcComponentID)
                            f.FormState = EnumFormState.FormAdd
                            f.ShowDialog()
                            Dim sOutput As String = f.TaskID
                            If sOutput <> "" Then
                                dtTaskID = Nothing
                                LoadTDBCTaskID(tdbcTaskID, ReturnValueC1Combo(tdbcComponentID))
                                tdbcTaskID.SelectedValue = sOutput
                                LoadTDBCTaskID(tdbcTaskIDM, ReturnValueC1Combo(tdbcComponentIDM), ReturnValueC1Combo(tdbcTaskIDM))
                                _bSaved = False 'phải khởi đầu biến này lại, nếu không sau khi thêm thành công, mở mh thêm mới tắt form mà k làm gì ->error
                                sTaskID = sOutput
                            Else
                                sTaskID = ""
                        ...
                        tdbcTaskID.Focus()
                    End If
                    tdbcTaskID.Tag = sTaskID
                End If
        End Select
    End Sub
    Private Sub LoadTDBGrid(Optional ByVal FlagAdd As Boolean = False, Optional ByVal sSubTaskID As String = "")
        If FlagAdd Then
        ...
        Dim sSQL As String = SQLStoreD45P1120()
        ...
        If sSubTaskID <> "" Then
            ...
            Dim dr() As DataRow = dt1.Select(COL_SubTaskID & "=" & SQLString(sSubTaskID), dt1.DefaultView.Sort)
    ...
    Private Sub LoadDataDefault()
        ...
        tdbcTaskID.SelectedValue = _taskID
        tdbcTaskID.Enabled = False
    End Sub
    ...
    Private Sub LoadAdd()
        ...
        If bCallFormAddNew Then
            ...
            UnReadOnlyControl(True, txtSubTaskID)
        Else
            UnReadOnlyControl(True, tdbcGroupProductID, tdbcComponentID, tdbcTaskID, txtSubTaskID)
        End If
    ...
    Private Sub LoadEdit()
        ...
        tdbg.Columns(COL_SubTaskID).Tag = tdbg(tdbg.Row, COL_SubTaskID).ToString
        '************************
        'Gán dữ liệu
        ...
        tdbcTaskID.SelectedValue = tdbg.Columns(COL_TaskID).Text
        txtSubTaskID.Text = tdbg.Columns(COL_SubTaskID).Text
        txtSubTaskName.Text = tdbg.Columns(COL_SubTaskName).Text
        txtNotes.Text = tdbg.Columns(COL_Notes).Text
        txtWorkerLevelID.Text = tdbg.Columns(COL_WorkerLevelID).Text
        cnePPUnitPrice.Value = Number(tdbg.Columns(COL_PPUnitPrice).Value)
        cnePPNorm.Value = Number(tdbg.Columns(COL_PPNorm).Value)
        chkDisabled.Checked = L3Bool(tdbg.Columns(COL_Disabled).Text)
        chkDisabled.Visible = True
        '************************
        ReadOnlyControl(tdbcMethodID, tdbcGroupProductID, tdbcComponentID, tdbcTaskID, txtSubTaskID)
    End Sub
...
#Region "Menu"
    ...
    Private Sub tsbDelete_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles tsbDelete.Click, mnsDelete.Click
        ...
        Dim sSQL As String = SQLDeleteD45T1120()
...
#Region "tdbg"
  ...
  If e IsNot Nothing AndAlso e.LastRow = -1 Then Exit Sub
        ...
        If tdbg.Columns(COL_SubTaskID).Tag Is Nothing OrElse tdbg.Columns(COL_SubTaskID).Tag.ToString <> tdbg(tdbg.Row, COL_SubTaskID).ToString Then
            LoadEdit()
        End If
    End Sub
...
#End Region
    ...
    Private Function SaveData(ByVal sender As System.Object) As Boolean
        ...
        Select Case _FormState
            Case EnumFormState.FormAdd
                sSQL.Append(SQLInsertD45T1120.ToString)
            Case EnumFormState.FormEdit
                sSQL.Append(SQLUpdateD45T1120.ToString)
        End Select
        ...
        If bRunSQL Then
            SaveOK()
            _bSaved = True
            _SubTaskID = txtSubTaskID.Text

            If bCallFormAddNew = False Then 'Chỉ làm khi lưới hiển thị
                Select Case _FormState
                    Case EnumFormState.FormAdd
                        LoadTDBGrid(True, txtSubTaskID.Text)
                    Case EnumFormState.FormEdit
                        LoadTDBGrid(, txtSubTaskID.Text)
                End Select
                ReadOnlyControl(tdbcMethodID, tdbcGroupProductID, tdbcComponentID, tdbcTaskID, txtSubTaskID)
                SetReturnFormView()
            Else
...
1:
    ...
    Private Function AllowSave(Optional bFilter As Boolean = False) As Boolean
        If bFilter Then
            ...
            If tdbcComponentIDM.Text.Trim = "" Then
                ...
                Return False
            End If
            If tdbcTaskIDM.Text.Trim = "" Then
                D99C0008.MsgNotYetChoose(lblTaskIDM.Text)
                tdbcTaskIDM.Focus()
                Return False
            End If
        Else
            ...
            If _FormState = EnumFormState.FormAdd AndAlso tdbcMethodID.Text.Trim <> "" Then  'Sinh mã tự động cho mã nhóm tiểu tác 
                ...
                If CheckStore(sSQL, "", dt) Then
                    If dt.Rows.Count > 0 Then txtSubTaskID.Text = dt.Rows(0).Item(COL_SubTaskID).ToString
                Else
                    Return False
                End If
            End If
            '******************************
            If txtSubTaskID.Text.Trim = "" Then
                D99C0008.MsgNotYetEnter(lblSubTaskID.Text)
                txtSubTaskID.Focus()
                Return False
            End If
            If txtSubTaskName.Text.Trim = "" Then
                D99C0008.MsgNotYetEnter(lblSubTaskName.Text)
                txtSubTaskName.Focus()
                Return False
            End If

            If _FormState = EnumFormState.FormAdd Then
                If IsExistKey("D45T1120", "SubTaskID", txtSubTaskID.Text) Then
    ...
    Private Function SQLInsertD09T6666() As StringBuilder
        ...
        sSQL.Append("UserID, HostID, Str01, Str02, Str03, Num01, FormID")
        ...
        sSQL.Append(SQLString(ReturnValueC1Combo(tdbcTaskID)) & COMMA) 'Str03, nvarchar[500], NOT NULL
    ...
    '# Created User: Nguyễn Lê Phương
    '# Created Date: 10/02/2017 11:20:09
    '#---------------------------------------------------------------------------------------------------
    Private Function SQLStoreD45P1120() As String
        Dim sSQL As String = ""
        ...
        sSQL &= SQLString(ReturnValueC1Combo(tdbcTaskIDM)) & COMMA 'TaskID, varchar[50], NOT NULL
    ...
    '#---------------------------------------------------------------------------------------------------
    '# Title: SQLInsertD45T1120
    '# Created User: Nguyễn Lê Phương
    '# Created Date: 10/02/2017 11:32:11
    '#---------------------------------------------------------------------------------------------------
    Private Function SQLInsertD45T1120() As StringBuilder
        Dim sSQL As New StringBuilder
        sSQL.Append("-- Luu them moi" & vbCrlf)
        sSQL.Append("Insert Into D45T1120(")
        sSQL.Append("GroupProductID, ComponentID, TaskID, SubTaskID, SubTaskNameU, " & vbCrlf)
        sSQL.Append("NotesU, Disabled, IGEMethodID, WorkerLevelID, CreateUserID, " & vbCrlf)
        ...
        sSQL.Append(SQLString(ReturnValueC1Combo(tdbcTaskID)) & COMMA) 'TaskID, varchar[50], NOT NULL
        sSQL.Append(SQLString(txtSubTaskID.Text) & COMMA) 'SubTaskID [KEY], varchar[50], NOT NULL
        sSQL.Append(SQLStringUnicode(txtSubTaskName, True) & COMMA & vbCrLf) 'SubTaskNameU, nvarchar[1000], NOT NULL
        ...
        sSQL.Append(SQLString(ReturnValueC1Combo(tdbcMethodID)) & COMMA) 'IGEMethodID, varchar[50], NOT NULL
        sSQL.Append(SQLString(txtWorkerLevelID.Text) & COMMA & vbCrLf) 'WorkerLevelID, varchar[50], NOT NULL
    ...
    '#---------------------------------------------------------------------------------------------------
    '# Title: SQLUpdateD45T1120
    '# Created User: Nguyễn Lê Phương
    '# Created Date: 10/02/2017 11:33:45
    '#---------------------------------------------------------------------------------------------------
    Private Function SQLUpdateD45T1120() As StringBuilder
        Dim sSQL As New StringBuilder
        sSQL.Append("-- Cap nhat du lieu" & vbCrlf)
        sSQL.Append("Update D45T1120 Set ")
        sSQL.Append("SubTaskNameU = " & SQLStringUnicode(txtSubTaskName, True) & COMMA) 'nvarchar[1000], NOT NULL
        ...
        sSQL.Append("WorkerLevelID = " & SQLString(txtWorkerLevelID.Text) & COMMA) 'varchar[50], NOT NULL
        ...
        sSQL.Append(" Where SubTaskID =" & SQLString(txtSubTaskID.Text))
        Return sSQL
    End Function

    '#---------------------------------------------------------------------------------------------------
    '# Title: SQLDeleteD45T1120
    '# Created User: Nguyễn Lê Phương
    '# Created Date: 10/02/2017 11:34:51
    '#---------------------------------------------------------------------------------------------------
    Private Function SQLDeleteD45T1120() As String
        Dim sSQL As String = ""
        sSQL &= ("-- Xoa du lieu" & vbCrlf)
        sSQL &= "Delete From D45T1120"
        sSQL &= " Where "
        sSQL &= "SubTaskID = " & SQLString(tdbg.Columns(COL_SubTaskID).Text)
        Return sSQL
    End Function
