...
#Region "Const of tdbg"
    ...
    Private Const COL_Disabled As Integer = 33              ' Không sử dụng
    Private Const COL_CreateUserID As Integer = 34          ' CreateUserID
    Private Const COL_CreateDate As Integer = 35            ' CreateDate
    Private Const COL_LastModifyUserID As Integer = 36      ' LastModifyUserID
    Private Const COL_LastModifyDate As Integer = 37        ' LastModifyDate
    Private Const COL_ReceivedDate As Integer = 38          ' Ngày nhận
    Private Const COL_BlockID As Integer = 39               ' BlockID
    Private Const COL_BlockName As Integer = 40             ' Khối
    Private Const COL_RecDepartmentID As Integer = 41       ' RecDepartmentID
    Private Const COL_RecDepartmentName As Integer = 42     ' Phòng ban
    Private Const COL_RecTeamID As Integer = 43             ' RecTeamID
    Private Const COL_RecTeamName As Integer = 44           ' Tổ nhóm
    Private Const COL_RecPositionID As Integer = 45         ' RecPositionID
    Private Const COL_RecPositionName As Integer = 46       ' Vị trí
    Private Const COL_ProjectID As Integer = 47             ' ProjectID
    Private Const COL_ProjectName As Integer = 48           ' Dự án
    Private Const COL_DesiredSalary As Integer = 49         ' Lương yêu cầu
    Private Const COL_CurrencyID As Integer = 50            ' CurrencyID
    Private Const COL_FileReceiverName As Integer = 51      ' Người nhận HS
    Private Const COL_ReceivedPlace As Integer = 52         ' Nơi nhận HS
    Private Const COL_RecSourceID As Integer = 53           ' RecSourceID
    Private Const COL_RecSourceName As Integer = 54         ' Nguồn tuyển dụng
    Private Const COL_Suggester As Integer = 55             ' Người giới thiệu
    Private Const COL_Sex As Integer = 56                   ' Sex
    Private Const COL_EmployeeID As Integer = 57            ' EmployeeID
#End Region
...
#Region "Const of tdbg1"
    ...
    Private Const COL1_SenAdjustID As Integer = 3     ' Tăng/Giảm thâm niên
...
#Region "Const of C1TrueDBGrid1"
    Private Const COL2_DepartmentName As Integer = 0    ' Phòng ban
    Private Const COL2_TeamName As Integer = 1          ' Tổ nhóm
    Private Const COL2_EmpGroupName As Integer = 2      ' Nhóm nhân viên
    Private Const COL2_DutyName As Integer = 3          ' Chức vụ
    Private Const COL2_WorkName As Integer = 4          ' Công việc/ Vị trí ứng tuyển
    Private Const COL2_InterviewFileName As Integer = 5 ' Lịch phỏng vấn
    Private Const COL2_IntDate As Integer = 6           ' Ngày PV
    Private Const COL2_InterviewLevels As Integer = 7   ' Vòng PV
    Private Const COL2_Content As Integer = 8           ' Nội dung PV
    Private Const COL2_Result As Integer = 9            ' Kết quả PV
    Private Const COL2_StatusID As Integer = 10         ' Trạng thái
#End Region
    ...
    Private Sub EnableMenu()
        If _isMSS = 0 Then Exit Sub
        tsbAdd.Visible = False
        tsmAdd.Visible = tsbAdd.Visible
        mnsAdd.Visible = tsbAdd.Visible

        tsbEdit.Visible = False
        tsmEdit.Visible = tsbEdit.Visible
        mnsEdit.Visible = tsbEdit.Visible

        tsbView.Visible = False
        tsmView.Visible = tsbView.Visible
        mnsView.Visible = tsbView.Visible

        tsbDelete.Visible = False
        tsmDelete.Visible = tsbDelete.Visible
        mnsDelete.Visible = tsbDelete.Visible

        tsbInherit.Visible = False
        tsmInherit.Visible = tsbInherit.Visible
        mnsInherit.Visible = tsbInherit.Visible

        tsbImportData.Visible = False
        tsmImportData.Visible = tsbImportData.Visible
        mnsImportData.Visible = tsbImportData.Visible
    End Sub
    ...
    Private Sub LoadTDBCombo()
        ...
        'Load tdbcBlockID
        LoadtdbcBlockID_D09P6868(tdbcBlockID, gsDivisionID, _formPermission, _isMSS)

        'Load tdbcDepartmentID
        dtDepartmentID = ReturnTableDepartmentID_D09P6868(gsDivisionID, _formPermission, _isMSS)
        'LoadtdbcDepartmentID(tdbcDepartmentID, dtDepartmentID, tdbcBlockID.SelectedValue.ToString, gbUnicode)

        'Load tdbcTeamID
        dtTeamID = ReturnTableTeamID_D09P6868(gsDivisionID, _formPermission, _isMSS)
    ...
    Private Sub Loadlanguage()
        ...
        '================================================================ 
        chkTransactionTypeID.Text = rL3("Su_dung_loai_nghiep_vu_HSNV") 'Sử dụng loại nghiệp vụ HSNV
        '================================================================ 
        ...
        '================================================================ 
        tdbg1.Columns(COL1_SenAdjustID).Caption = rL3("TangGiam_tham_nien") 'Tăng/Giảm thâm niên
...
#End Region
    ...
    '    sSQL = " Delete D25T1041 Where DivisionID = " & SQLString(gsDivisionID)
    '    sSQL &= "And CandidateID = " & SQLString(tdbg.Columns(COL_CandidateID).Text) & vbCrLf

    '    sSQL &= " Delete D25T1042 Where DivisionID = " & SQLString(gsDivisionID)
    '    sSQL &= " And CandidateID = " & SQLString(tdbg.Columns(COL_CandidateID).Text) & vbCrLf

    '    sSQL &= " Delete D25T1050 Where DivisionID = " & SQLString(gsDivisionID)
    '    sSQL &= " And CandidateID =" & SQLString(tdbg.Columns(COL_CandidateID).Text) & vbCrLf

    '    sSQL &= " Delete D25T1051 Where DivisionID = " & SQLString(gsDivisionID)
    '    sSQL &= " And CandidateID = " & SQLString(tdbg.Columns(COL_CandidateID).Text) & vbCrLf

    '    sSQL &= " Delete D25T1052 Where DivisionID = " & SQLString(gsDivisionID)
    '    sSQL &= " And CandidateID = " & SQLString(tdbg.Columns(COL_CandidateID).Text) & vbCrLf

    '    sSQL &= " Delete D91T1010 Where DivisionID = " & SQLString(gsDivisionID)
    '    'sSQL &= " And CandidateID =" & tdbg.Columns(COL_CandidateID).Text & vbCrLf
    '    sSQL &= " And TableName = 'D25T1041'" & vbCrLf

    '    'add 15/01/08
    '    sSQL &= " Delete D25T2011 Where DivisionID = " & SQLString(gsDivisionID)
    '    sSQL &= " And CandidateID = " & SQLString(tdbg.Columns(COL_CandidateID).Text) & vbCrLf

    '    sSQL &= " Delete D25T1054 Where DivisionID = " & SQLString(gsDivisionID)
    '    sSQL &= " And CandidateID = " & SQLString(tdbg.Columns(COL_CandidateID).Text) & vbCrLf
    ...
    '        Dim Decs4 As String = ""
    '        Dim Decs5 As String = ""
    '        Decs1 = Trim(gsDivisionID.ToString)
    ...
    Private Sub tsbPrint_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles tsbPrint.Click, tsmPrint.Click, mnsPrint.Click
        ...
        CallFormShow(Me, "D25D0340", "D25F4000", arrPro) 'ByVal isMSS As Integer, ByVal sFind As String, ByVal visibleFilterButton As String
    ...
    'Them ngay 9/1/2013 theo ID 50600 của Phương Thảo bởi Văn Vinh
    ...
    Private Sub ReLoadTDBGrid1()
        ...
        FooterTotalGrid(tdbg1, COL1_SenAdjustID)
    End Sub
